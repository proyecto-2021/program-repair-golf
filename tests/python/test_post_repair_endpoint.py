...
def test_post_repair_challenge_invalid(client, jwt_token):
    
    repair_request = request_creator(code_path = examples_path + "code_repair_2.py")
    
    invalid_id = 1000000000000000

    response = client.post(api_url + '/' + str(invalid_id) + '/repair', data = repair_request, headers={'Authorization': f'JWT {jwt_token}'})
    
    assert response.status_code == 409
    assert response.json['Error'] == "Challenge not found"

def test_post_repair_code_not_provided(client, jwt_token):
    
    repair_objectiveParam = "Test repair"
    post_info = send_post(client, jwt_token, "valid_code_16.py", "valid_atest_16.py", repair_objectiveParam, '3')
    challenge_id = post_info.json['challenge']['id']
    
    response = client.post(api_url + '/' + str(challenge_id) + '/repair', headers={'Authorization': f'JWT {jwt_token}'})
...
def test_post_repair_update_best_score(client, jwt_token):
    ...
    response = client.post(api_url + '/' + str(challenge_id) + '/repair', data = repair_request_2, headers={'Authorization': f'JWT {jwt_token}'})

    assert response.status_code == 200
    assert response.json['repair']['challenge']['best_score'] >= old_best_score

def test_post_repair_not_past_tests(client, jwt_token):
    
    repair_objectiveParam = "Test repair"
    post_info = send_post(client, jwt_token, "valid_code_18.py", "valid_atest_18.py", repair_objectiveParam, '3')
